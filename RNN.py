...
def create_model(X, embedding_matrix, embedding_dim, tag_tokenizer):
    ...
    model.add(
        tf.keras.layers.Embedding(input_dim=len(embedding_matrix), output_dim=embedding_dim, weights=[embedding_matrix],
                                  input_length=X.shape[1], trainable=True))
    model.add(tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(32, return_sequences=True)))
    model.add(tf.keras.layers.Dropout(0.3))  
    model.add(tf.keras.layers.Dense(len(tag_tokenizer.word_index) + 1, activation='softmax'))
    model.compile(loss='sparse_categorical_crossentropy', optimizer=tf.keras.optimizers.Adam(learning_rate=0.001))
    return model


def train_model(model, X_train, y_train):
    checkpoint = tf.keras.callbacks.ModelCheckpoint('Model/RNN-model.h5', monitor='val_loss', verbose=1,
    ...
    plt.savefig('Model/rnn-train.png')
    plt.show()


def evaluate_model(X_test, y_test, word_tokenizer, tag_tokenizer):
    model = tf.keras.saving.load_model('Model/RNN-model.h5')
    ...
    with open('Model/Train_output/rnn-train.txt', 'w') as f:
        flattened_y_pred = []
        flattened_y_true = []
...
if __name__ == '__main__':
    ...
    # store tokenizer
    with open('Model/rnn_word_tokenizer.pkl', 'wb') as f:
        pickle.dump(word_tokenizer, f)
    with open('Model/rnn_tag_tokenizer.pkl', 'wb') as f:
